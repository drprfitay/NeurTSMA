...
internal_validation = {BIN    : {'expected_params' : ["bin_size"],  'operation' : "binning matrix"},\
                       ...
                       #REDUCE : {},\ ToDo: trash remove from code
                       CUSTOM : {'expected_params' : ["function", "prefix"]}}


scaling_methods = {"zscore" : lambda vec: (vec - np.mean(vec))/(np.std(vec)),\
...
def _initialize_environment(base_directory):        
...
def _create_dataset(matrix=None, dataset_name=None):        
...
def _get_dataset(dataset_name, alias=None, prefix=None, raise_exception=True):
    dataset_directory = "%s\\%s" % (_get_base_directory(), dataset_name)
...
def _process_dataset(dataset_name=None, alias=None, prefix=None, op_list=list(), rebase_alias=False):    
    global _verbose
    
    mat = _get_dataset(dataset_name, alias=alias, prefix=prefix)    
    ...
    for op in op_list:
        ...
        intermediate_dataset = _get_dataset(dataset_name, alias=alias, prefix=intermediate_prefix, raise_exception=False)
...
def _process_cluster_map(dataset_name, 
                         alias=None,
                         prefix=None,
       ...
       # Include only the #N farthest away timepoints
        ...
        for num_of_clusters in range(2,max_cluster_number + 1):            
            ...
            for rep_i in range(0,nreps):
                kmeans = KMeans(n_clusters=num_of_clusters, n_init="auto") # what the fuck are these warnings
                kmeans.fit_predict(np.transpose(included_pointcloud_mat))                
